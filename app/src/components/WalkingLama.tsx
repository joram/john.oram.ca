...
const WalkingLama: React.FC = () => {
  const { isLamaEnabled } = useLama();
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const animationRef = useRef<number>();
  const [animation, setAnimation] = useState<LamaAnimation | null>(null);
  ...
  useEffect(() => {
    ...
    // Start a new animation when lama mode is enabled
